...
BRANDED_LOOKUP_BUDGET_SECONDS = 6.0
PROVIDER_ADAPTERS: dict[str, tuple[Any, bool]] = {}
...
def register_provider_adapter(source: str, adapter, *, timeout_aware: bool = False) -> None:
    """Register one lookup adapter through the explicit test/provider seam.

    Timeout-aware adapters receive ``(query, remaining_seconds)``. Legacy
    adapters receive ``(query)`` exactly once and are best-effort.
    """
    PROVIDER_ADAPTERS[str(source)] = (adapter, bool(timeout_aware))


def _provider_warning(provider: str, reason: str) -> None:
    logger.warning("%s provider warning: %s", provider, reason)


def _call_with_remaining_timeout(callback, query: str, remaining: float):
    with food_provider_transport.remaining_timeout(remaining):
        return callback(query)


def _invoke_provider(source: str, query: str, deadline: float) -> tuple[Any, bool]:
    entry = PROVIDER_ADAPTERS.get(source)
    if not entry:
        return None, False
    if callable(entry):
        adapter, timeout_aware = entry, False
    else:
        adapter, timeout_aware = entry
    remaining = deadline - time.monotonic()
    if remaining <= 0:
        return None, True
    try:
        result = adapter(query, remaining) if timeout_aware else adapter(query)
    except Exception:
        _provider_warning(source, "provider failure")
        return None, time.monotonic() >= deadline
    return result, time.monotonic() >= deadline
...
def lookup(
    ...
    *,
    ...
    deadline = time.monotonic() + BRANDED_LOOKUP_BUDGET_SECONDS
    ...
    for source in priorities:
        if source == "cache":
            try:
                ...
                _provider_warning("cache", "read failure")
                cached = None
            if cached and _cache_allowed_for_lookup(cached, private_label_brand):
                return cached
            continue
        estimate, exhausted = _invoke_provider(source, lookup_text, deadline)
        if estimate:
            if isinstance(estimate, dict) and estimate.get("source"):
                _save_cache_best_effort(normalized, estimate["source"], estimate, user_id=user_id)
            return estimate
        if exhausted:
            break
    return None
    ...
    for source in BARCODE_SOURCE_PRIORITY:
        if source == "cache":
            ...
            if cached:
                return cached
            continue
        estimate, exhausted = _invoke_provider(source, normalized, deadline)
        if estimate:
            if isinstance(estimate, dict) and estimate.get("source"):
...
def _save_cache_best_effort(normalized: str, source: str, estimate: dict[str, Any], *, user_id: int) -> None:
    try:
        ...
        _provider_warning("cache", "write failure")
        return
...
def _fill_off_cache_verified_url(estimate: dict[str, Any]) -> None:
...
_fill_off_barcode_cache_verified_url = _fill_off_cache_verified_url
...
def _nutritionix_lookup(text: str, normalized: str) -> dict[str, Any] | None:
    ...
    if payload is None:
        return None
    if not isinstance(payload, dict):
        _provider_warning("nutritionix", "malformed provider response")
        return None
...
def _nutritionix_barcode_lookup(barcode: str) -> dict[str, Any] | None:
    ...
    if payload is None:
        return None
    if not isinstance(payload, dict):
        _provider_warning("nutritionix_barcode", "malformed provider response")
        return None
    foods = payload.get("foods")
    if not isinstance(foods, list):
        _provider_warning("nutritionix_barcode", "malformed provider response")
...
def _usda_barcode_lookup(barcode: str) -> dict[str, Any] | None:
    ...
    if payload is None:
        return None
    if not isinstance(payload, dict):
        _provider_warning("usda_fdc_barcode", "malformed provider response")
        return None
    foods = payload.get("foods")
    if not isinstance(foods, list):
        _provider_warning("usda_fdc_barcode", "malformed provider response")
    ...
    if not foods:
        return None
    for food, nutrients, calories in _usable_usda_candidates(
        [food for food in foods if isinstance(food, dict)], barcode=barcode
    ):
...
def _usda_lookup(text: str, normalized: str) -> dict[str, Any] | None:
    ...
    if payload is None:
        return None
    if not isinstance(payload, dict):
        _provider_warning("usda_fdc", "malformed provider response")
        return None
    foods = payload.get("foods")
    if not isinstance(foods, list):
        _provider_warning("usda_fdc", "malformed provider response")
...
def _usable_usda_candidates(
    foods: list[dict[str, Any]], *, barcode: str | None = None
):
    candidates = _matching_usda_barcode_foods(foods, barcode) if barcode else foods
    for food in candidates:
        raw_nutrients = food.get("foodNutrients")
        if not isinstance(raw_nutrients, list):
            continue
        food_nutrients = [n for n in raw_nutrients if isinstance(n, dict)]
...
def _open_food_facts_lookup(text: str) -> dict[str, Any] | None:
    ...
    if payload is None:
        return None
    if not isinstance(payload, dict):
        _provider_warning("open_food_facts", "malformed provider response")
        return None
    ...
    *,
    ...
    if reject_us_only:
        ...
        normalized_countries = {
            ...
            and re.fullmatch(r"[a-z]{2}:[a-z0-9]+(?:-[a-z0-9]+)*", country.strip().lower())
        }
        return any(country != "en:united-states" for country in normalized_countries)
...
def _open_food_facts_estimate(product: dict[str, Any]) -> dict[str, Any]:
    ...
    estimate = {
        ...
        "portion_description": "100 g",
        "meal_type": "snack",
        "calories": _off_number_or_none(_off_energy_kcal(nutriments)),
        "protein_g": _off_number_or_none(nutriments.get("proteins_100g")),
        "carbs_g": _off_number_or_none(nutriments.get("carbohydrates_100g")),
        "fat_g": _off_number_or_none(nutriments.get("fat_100g")),
        ...
        "external_food_id": str(product["code"]) if product.get("code") is not None else None,
        "verified_source_url": _off_verified_source_url(product),
        "data_fetched_at": datetime.now().isoformat(timespec="seconds"),
...
def _sanitize_with_provenance(estimate: dict[str, Any]) -> dict[str, Any]:
    ...
    for key in (*PUBLIC_PROVENANCE_FIELDS, "source_brand_name"):
        if key not in sanitized and estimate.get(key) is not None:
...
def _off_quality_ok(product: dict[str, Any]) -> bool:
    ...
    if not macros_present_and_plausible:
        return False
    if not _off_optional_nutrients_plausible(nutriments):
        return False
...
def _off_optional_nutrients_plausible(nutriments: dict[str, Any]) -> bool:
    for key in ("sodium_100g", "salt_100g"):
        raw_value = nutriments.get(key)
        ...
        if not math.isfinite(numeric_value):
            return False
        maximum = SODIUM_MG_MAX / (1000 if key == "sodium_100g" else 393.4)
        if not 0 <= numeric_value <= maximum:
            return False
    sodium_mg = _off_sodium_mg(nutriments)
    if sodium_mg is not None and not 0 <= sodium_mg <= SODIUM_MG_MAX:
        return False
    ...
    if fiber is not None:
        try:
            ...
            fiber_value = None
        if fiber_value is not None and (
            not math.isfinite(fiber_value) or not 0 <= fiber_value <= MACRO_GRAM_MAX
        ):
            return False
    return True


def _off_verified_source_url(product: dict[str, Any]) -> str:
    value = product.get("url")
    return value.strip() if isinstance(value, str) and value.strip() else "https://world.openfoodfacts.org/"
...
def _off_sodium_mg(nutriments: dict[str, Any]) -> int | None:
...
def _off_serving_sodium_mg(nutriments: dict[str, Any], *, serving_quantity_g: float | None = None) -> int | None:
...
def _sum_nutritionix(foods: list[dict[str, Any]], key: str) -> float | None:
    total = 0.0
    unknown = False
    for food in foods:
        value = food.get(key)
...
def _nutritionix_text_adapter(query: str, remaining: float):
    normalized = normalize_meal_text(query)
    return _call_with_remaining_timeout(
        lambda value: _nutritionix_lookup(value, normalized),
        query,
        remaining,
    )


def _usda_text_adapter(query: str, remaining: float):
    normalized = normalize_meal_text(query)
    return _call_with_remaining_timeout(
        lambda value: _usda_lookup(value, normalized),
        query,
        remaining,
    )


def _open_food_facts_text_adapter(query: str, remaining: float):
    return _call_with_remaining_timeout(
...
def _nutritionix_barcode_adapter(barcode: str, remaining: float):
    return _call_with_remaining_timeout(
...
def _usda_barcode_adapter(barcode: str, remaining: float):
    return _call_with_remaining_timeout(
        _usda_barcode_lookup,
        barcode,
        remaining,
    )


def _open_food_facts_barcode_adapter(barcode: str, remaining: float):
    return _call_with_remaining_timeout(
        _open_food_facts_barcode_lookup,
        barcode,
        remaining,
    )


register_provider_adapter("heb_product_page", heb_product_lookup.lookup)
register_provider_adapter("nutritionix", _nutritionix_text_adapter, timeout_aware=True)
register_provider_adapter("usda_fdc", _usda_text_adapter, timeout_aware=True)
register_provider_adapter("open_food_facts", _open_food_facts_text_adapter, timeout_aware=True)
register_provider_adapter("nutritionix_barcode", _nutritionix_barcode_adapter, timeout_aware=True)
register_provider_adapter("usda_fdc_barcode", _usda_barcode_adapter, timeout_aware=True)
register_provider_adapter("open_food_facts_barcode", _open_food_facts_barcode_adapter, timeout_aware=True)
