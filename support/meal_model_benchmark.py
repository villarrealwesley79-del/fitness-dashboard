...
ADJUST_INTENT_CASES = [
    ...
    _adjust_case(
        2,
        "Make today's workout lighter because I have a busy day and want to leave some energy in reserve.",
        {"sets_delta_pct_range": (-20, -1)},
        readiness={"sleep_hours": 7, "soreness": "low", "hrv_status": "normal"},
        plan=_compact_plan(focus="full body", estimated_minutes=55),
        coverage=("make-lighter", "set-reduction"),
    ),
    _adjust_case(
        3,
        "Avoid loading my right shoulder today because it is irritated.",
        {"avoid_joints": [{"side": "right", "joint": "shoulder"}]},
        readiness={"sleep_hours": 8, "soreness": "low", "right_shoulder": "irritated"},
        coverage=("shoulder-avoidance", "joint-specific"),
    ),
    _adjust_case(
        4,
        "My left knee is sore, so avoid exercises that load that knee.",
        {"avoid_joints": [{"side": "left", "joint": "knee"}]},
        readiness={"sleep_hours": 7, "soreness": "left knee 5/10", "hrv_status": "normal"},
        coverage=("knee-avoidance", "joint-specific"),
    ),
    _adjust_case(
        5,
        "Avoid movements that load my low back today; it feels tight after yesterday's work.",
        {"avoid_joints": [{"side": "both", "joint": "spine"}]},
        readiness={"sleep_hours": 7, "soreness": "low-back tightness", "hrv_status": "normal"},
        coverage=("low-back-avoidance", "spine"),
    ),
    _adjust_case(
        6,
        "Drop cardio from today's lifting session; I only want the strength work.",
        {"drop_cardio": True},
        readiness={"sleep_hours": 8, "soreness": "low", "hrv_status": "normal"},
        plan=_compact_plan(cardio="intervals", estimated_minutes=60),
        coverage=("drop-cardio", "explicit"),
    ),
    _adjust_case(
        7,
        "Combine the lower-body sets and lower the RPE because my legs are unusually fatigued.",
        {"sets_delta_pct_range": (-20, -1), "rpe_delta_range": (-1, -0.1)},
        readiness={"sleep_hours": 6, "soreness": "legs 6/10", "hrv_status": "down"},
        plan=_compact_plan(focus="lower body", estimated_minutes=50),
        coverage=("lower-sets", "rpe-reduction", "combined"),
    ),
    _adjust_case(
        8,
        "Keep my upper-body work, but avoid training my legs today because they are still sore.",
        {"avoid_muscles": ["legs"]},
        readiness={"sleep_hours": 8, "soreness": "legs 7/10", "hrv_status": "normal"},
        plan=_compact_plan(focus="upper body", estimated_minutes=45),
        coverage=("preserve-upper", "avoid-legs"),
    ),
    _adjust_case(
        9,
        "Take it easy today.",
        {"rpe_delta_range": (-1, -0.1)},
        readiness={"sleep_hours": 6, "soreness": "moderate", "hrv_status": "unknown"},
        coverage=("ambiguous-easy", "conservative"),
    ),
    _adjust_case(
        10,
        "I am traveling with only dumbbells and a bench, so swap the barbell back squat for a suitable leg exercise.",
        {
            "swap": [{
                "replace_exercise": "Back Squat",
                "target_muscle": "quadriceps",
                "target_exercise": "Dumbbell Goblet Squat",
                "reason": "available travel equipment",
            }],
        },
        readiness={"sleep_hours": 7, "soreness": "low", "equipment": "dumbbells and bench"},
        plan=_compact_plan(
            focus="lower body",
            exercises=[{
                "exercise": "Back Squat",
        ...
        coverage=("travel-equipment", "swap"),
    ),
    _adjust_case(
        11,
        "Shorten today's session to 30 minutes while keeping the most important work.",
        {"duration_cap_min": 30},
        readiness={"sleep_hours": 7, "soreness": "low", "available_minutes": 30},
        plan=_compact_plan(estimated_minutes=60),
        coverage=("shortened-session", "duration-cap"),
    ),
    _adjust_case(
        12,
        "Avoid overhead pressing today because overhead positions bother my shoulders.",
        {"avoid_joints": [{"side": "both", "joint": "shoulder"}]},
        readiness={"sleep_hours": 8, "soreness": "shoulders 3/10", "hrv_status": "normal"},
        coverage=("overhead-pressing", "shoulder-avoidance"),
    ),
    _adjust_case(
        13,
        "Avoid exercises that load my elbows; they feel irritated after pressing and curls.",
        {"avoid_joints": [{"side": "both", "joint": "elbow"}]},
        readiness={"sleep_hours": 7, "soreness": "elbows 4/10", "hrv_status": "normal"},
        coverage=("elbow-avoidance", "joint-specific"),
    ),
    _adjust_case(
        14,
        "Swap the named Back Squat for Leg Press while keeping the quadriceps focus.",
        {
            "swap": [{
                "replace_exercise": "Back Squat",
                "target_muscle": "quadriceps",
                "target_exercise": "Leg Press",
                "reason": "requested named exercise swap",
            }],
        },
        readiness={"sleep_hours": 8, "soreness": "low", "hrv_status": "normal"},
        plan=_compact_plan(
        ...
        coverage=("named-exercise-swap", "swap"),
    ),
    _adjust_case(
        15,
        "I am fatigued but not injured; keep the session pattern and reduce how hard the sets feel.",
        {"rpe_delta_range": (-1, -0.1)},
        readiness={"sleep_hours": 6, "soreness": "moderate", "injury": False},
        coverage=("fatigue-without-injury", "rpe-reduction"),
    ),
    _adjust_case(
        16,
        "Keep cardio in the session, but make it lower intensity than usual.",
        {"drop_cardio": False},
        readiness={"sleep_hours": 8, "soreness": "low", "cardio_readiness": "moderate"},
        plan=_compact_plan(cardio="intervals"),
        coverage=("keep-cardio", "lower-intensity"),
    ),
    _adjust_case(
        17,
        "Make today a recovery day rather than a hard training day.",
        {"sets_delta_pct_range": (-20, -1), "rpe_delta_range": (-1, -0.1)},
        readiness={"sleep_hours": 5, "soreness": "high", "hrv_status": "down"},
        coverage=("recovery-day", "conservative"),
    ),
    _adjust_case(
        18,
        "I only want compound lifts today; remove isolation work from the plan.",
        {"avoid_muscles": ["isolation"]},
        readiness={"sleep_hours": 8, "soreness": "low", "hrv_status": "normal"},
        plan=_compact_plan(
            focus="full body",
            exercises=[
                {"exercise": "Bench Press", "muscle": "chest", "is_compound": True, "target_sets": 4, "target_reps": 6, "rpe_target": 8},
                {"exercise": "Biceps Curl", "muscle": "biceps", "is_compound": False, "target_sets": 3, "target_reps": 10, "rpe_target": 7},
            ],
        ),
        coverage=("compound-only", "remove-isolation"),
    ),
    _adjust_case(
        19,
        "Remove shoulder work from today's plan, but do not invent any other changes.",
        {"avoid_muscles": ["shoulders"]},
        readiness={"sleep_hours": 8, "soreness": "shoulders 5/10", "hrv_status": "normal"},
        coverage=("remove-muscle-group", "avoid-muscle"),
    ),
    _adjust_case(
        20,
        "No changes are needed today; keep the current plan exactly as it is.",
        {
            "avoid_muscles": [],
            "avoid_joints": [],
            "swap": [],
        ...
        readiness={"sleep_hours": 8, "soreness": "low", "hrv_status": "normal"},
        coverage=("no-op", "no-invented-changes"),
    ),
]
...
SWAP_RESOLUTION_CASES = [
    ...
    _swap_case(
        ...
        current="Bench Press",
        muscle="chest",
        coverage=("common-nickname", "compound"),
    ),
    _swap_case(
        6,
        "seated cable row",
    ...
    _swap_case(
        ...
        "Cabl Row",
        "Cable Row",
        current="Dumbbell Row",
        muscle="back",
        coverage=("one-character-typo",),
    ),
    _swap_case(
        8,
        "Cabel Row",
    ...
    _swap_case(
        ...
        coverage=("unambiguous-compound-nickname", "single-candidate"),
    ),
    _swap_case(
        11,
        "incline db press",
        ...
        coverage=("multiple-candidates", "single-alias-match"),
    ),
    _swap_case(
        12,
        "",
    ...
    _swap_case(
        ...
        current="Bench Press",
        muscle="full body",
        coverage=("unrelated-out-of-library", "no-match"),
    ),
    _swap_case(
        14,
        "Dumbbell Fly",
        ...
        current="Bench Press",
        muscle="chest",
        coverage=("plausible-absent-exercise", "no-match"),
    ),
    _swap_case(
        15,
        "press",
        ...
        current="Bench Press",
        muscle="chest",
        coverage=("ambiguous-generic-term", "no-match"),
    ),
]
...
POST_WORKOUT_ANALYSIS_CASES = [
    ...
    _analysis_case(
        ...
        _compact_workout(
            name="Back Squat",
            muscle="quadriceps",
            notes="last reps were slow",
            rpe=9,
            session_type="lower strength",
            total_sets=4,
            total_volume_lbs=2000,
        ),
        {
            "sleep_hours": 6,
            "soreness": "moderate",
            "calories": "slightly under target",
            "recent_sessions": [
                {"exercise": "Back Squat", "rpe": 7},
                {"exercise": "Back Squat", "rpe": 7.5},
            ],
        },
        {
            "summary": (("rpe", "effort"), ("squat",)),
            "wins": (("completed", "squat", "session"),),
            "concerns": (("fatigue", "hard", "high"), ("rpe", "effort")),
            "comparison": (("higher", "up", "regression"),),
            "next_session_cue": (("recover", "form", "readiness"),),
        },
        ("high-rpe-regression", "concern", "comparison"),
    ),
    _analysis_case(
        3,
        _compact_workout(
            ...
            notes="steady reps",
            rpe=7,
            session_type="upper strength",
            total_sets=5,
            total_volume_lbs=2500,
        ),
        {
            "sleep_hours": 8,
            "soreness": "low",
            "calories": "on target",
            "recent_sessions": [
                {"exercise": "Bench Press", "rpe": 7, "total_volume_lbs": 2000},
                {"exercise": "Bench Press", "rpe": 7, "total_volume_lbs": 2250},
            ],
        },
        {
            "summary": (("progress", "volume", "strong"), ("bench", "press")),
            "wins": (("volume", "bench", "press"), ("completed", "progress")),
            "concerns": (),
            "comparison": (("up", "more", "progress"),),
            "next_session_cue": (("progress", "form", "technique"),),
            "empty_fields": ("concerns",),
        },
        ("volume-progression", "win", "empty-concerns"),
    ),
    _analysis_case(
        4,
        _compact_workout(
            name="Incline Dumbbell Press",
            muscle="chest",
            notes="right shoulder pain during the final set",
            rpe=7,
            session_type="upper hypertrophy",
            total_volume_lbs=1500,
        ),
        {
            ...
            "soreness": "low",
            "calories": "on target",
            "recent_sessions": [{"exercise": "Incline Dumbbell Press", "rpe": 7}],
        },
        {
            "summary": (("pain", "shoulder"), ("press", "session")),
            "wins": (("completed", "press", "reps"),),
            "concerns": (("pain",), ("right", "shoulder")),
            "comparison": (("steady", "same", "history"),),
            "next_session_cue": (("pain", "shoulder", "stop", "avoid", "monitor"),),
        },
        ("pain-note", "safety-cue", "right-shoulder"),
    ),
    _analysis_case(
        5,
        _compact_workout(
            ...
            session_type="lower strength",
            total_sets=3,
            total_volume_lbs=1800,
        ),
        {
            "sleep_hours": 4,
            "soreness": "high",
            "calories": "below target",
            "recent_sessions": [{"exercise": "Deadlift", "rpe": 7}],
        },
        {
            "summary": (("under", "recovered", "fatigue", "sleep"), ("deadlift", "session")),
            "wins": (("completed", "deadlift", "controlled"),),
            "concerns": (("sleep", "soreness", "fatigue", "recovery"),),
            "comparison": (("higher", "harder", "up", "under"),),
            "next_session_cue": (("recover", "rest", "readiness", "form"),),
        },
        ("under-recovered", "conservative-cue", "recovery"),
    ),
]
