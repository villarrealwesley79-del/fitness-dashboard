...
Markdown export: `/api/export-md` streams a `text/markdown` attachment named `workout_export.md`. It includes a summary and one table row per workout set. Missing, non-finite, non-scalar, or overflowed strength values are rendered as `N/A`, and backslashes, table delimiters, or newlines in scalar values are escaped. Exercises without sets use `Non-strength/watch-only row` only for canonical watch sources; other partial exercises use `No set data`. An exercise-less workout preserves its session type or canonical source in the Machine column and otherwise uses `No exercise data`. Invalid top-level workouts, exercise or set collections, and invalid nested values are labeled rather than aborting or disappearing from the export.
