...
This module keeps route code out of the details of projecting hub payloads into
status, wearable facts, recommendation markers, and recommendation sources.
"""
...
def status_source(status_payload=None) -> dict:
...
def store_wearable_facts(
    ...
    *,
    ...
    if facts:
        upsert_daily_facts(db_file, facts)
    return len(facts)


def recommendation_facts(db_file: str, limit: int = 20) -> list[dict]:
    return [
        fact for fact in list_recommendation_facts(db_file, limit=limit)
...
def recommendation_source_payload(freshness, *, db_file: str, facts=None, modifier=None) -> dict:
    facts = recommendation_facts(db_file) if facts is None else facts
