...
Bob's architecture (2026-04-23):
    HQ Flask app
      -> POST compact context JSON
      -> ASUS LM Studio primary, Mac LM Studio fallback
      -> strict JSON patch back
      -> HQ Python validates/applies, never the other way around.
...
LM_STUDIO_PRIMARY_URL = os.environ.get(
    ...
    os.environ.get("LM_STUDIO_URL", "http://100.99.46.2:1234"),
)
...
# Bob: "Concurrency: 1 adjustment request at a time." A global semaphore keeps
...
def _preflight_candidate(candidate: dict) -> None:
    ...
    ASUS can be offline while Mac fallback is healthy. Without this preflight, a
    request spends the full generation timeout on ASUS before it even tries Mac.
    """
    try:
