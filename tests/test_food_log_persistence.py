...
def test_orphaned_bulk_deletion_helpers_are_not_exposed(isolated_store):
    store, _ = isolated_store

    assert not hasattr(store, "clear_food_logs")
    assert not hasattr(store, "delete_user_data")
